    ...
    var body: some View {
        ScrollView {
            MapView(coordinate: landmark.locationCoordinate)
                ...
                .overlay(
                    GeometryReader { proxy in
                        Button("Open in Maps") {
                            let dest = MKMapItem(placemark: MKPlacemark(coordinate: self.landmark.locationCoordinate))
                            dest.name = self.landmark.name
                            dest.openInMaps()
                        }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text(landmark.description).lineLimit(nil)
            }
            .padding()
            .frame(maxWidth: 700)
            .offset(x: 0, y: -50)
